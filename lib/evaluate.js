...
function evaluateSum(scope, expression) {
  var errors = []
  var children = expression.children
  var accum = 0
  ...
  for (var i = 0, e = children.length; i < e; i += 1) {
    ...
    if (child.type !== 'Number') {
      errors = errors.concat(
        expression._type + ' children must be Number type'
      )
    }
...
function evaluateEqual(scope, expression) {
  var errors = []
  var children = expression.children
  var accum = true
  var value = undefined
...
function evaluate(scope, expression) {
  switch (expression._type) {
    ...
    case 'Sum':
      return evaluateSum(scope, expression)

    case 'Equal':
      return evaluateEqual(scope, expression)
